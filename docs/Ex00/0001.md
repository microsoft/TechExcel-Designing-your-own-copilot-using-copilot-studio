...
{: .important }
> For those who are building a demo environment using CDX, you can start with a 90 day M365. Start by going to **cdx.transform.microsoft.com** and select **My Environments**. Click **Create Tenant** on the first option *Microsoft M365 demo content* and accept the diaclaimer, wait for the provisioning.  You will now see a new M365 tenant in **My Environments**. Open an In Private browser, sign in to *Office.com* with the new credentials and activate MFA.  Consult your instructor if you need support.
> 
